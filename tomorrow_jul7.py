...
def predict(ta,tb,oh,od,oa,alt=0):
    ...
    # 高原
    if alt>=1500:
        hf=1+(alt-1500)*0.0015/100
        af=max(1-(alt-1500)*0.0005/100,0.92)
        la*=hf; lb*=af
    ...
    star_a=STAR_FACTOR.get(ta,0)*100
    star_b=STAR_FACTOR.get(tb,0)*100
    
    print("="*55)
    print(f"  V5.5 預測：{ta} vs {tb}  🏆 8強")
    if alt>0: print(f"  🏔️ 海拔: {alt}m")
    if star_a>0 or star_b>0:
        print(f"  ⭐ 加成: {ta}+{star_a:.0f}% / {tb}+{star_b:.0f}%")
    print("="*55)
...
predict("葡萄牙","西班牙",2.80,3.10,2.50)
predict("美國","比利時",2.30,3.20,3.10)
